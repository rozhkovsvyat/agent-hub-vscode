...
import { vendorBridgeHostCwd, vendorBridgeHostExtensionInfo } from "./vendorBridgeHost";
...
export async function* streamBridgeChat(
  ...
  const cwd = vendorBridgeHostCwd() ?? process.cwd();
...
async function* streamBridgeChatWithSteer(
  ...
  try {
    ...
    const extensionInfo = vendorBridgeHostExtensionInfo();
    const extensionBinding = extensionInfo
      ? runtimeCanaryExtensionBinding(
